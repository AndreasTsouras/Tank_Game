...
{
    std::vector<Vector3f> new_positions;
    for (const auto& position : positions)
    {
        new_positions.push_back(Vector3f(position.x + translation.x, position.y + translation.y, position.z + translation.z));
    }
    positions = new_positions;

    // Update the OpenGL vertex buffer object
    GLuint buffer;
    glGenBuffers(1, &buffer);
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glBufferData(GL_ARRAY_BUFFER, positions.size() * sizeof(Vector3f), positions.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);
    glBufferData(GL_ARRAY_BUFFER, positions.size() * sizeof(Vector3f), positions.data(), GL_STATIC_DRAW);
}
